  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDateFns}>
      ...
      >
        ...
        <DialogContent dividers sx={{ p: 3 }}>
          {!config ? (
            <Box sx={{ py: 4, textAlign: 'center' }}>
              ...
              <Typography variant="body2" color="text.secondary">
                Loading risk matrix configuration...
              </Typography>
            </Box>
          ) : (
            <Stack spacing={3}>
              ...
              <AccordionDetails>
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12}>
                    <Alert 
                      severity={
                        config && residualRiskLevel <= config.low_threshold 
                          ? 'success' 
                          : config && residualRiskLevel <= config.medium_threshold 
                          ? 'warning' 
                          : 'error'
                      }
                    >
        ...
        <DialogActions sx={{ p: 2.5, borderBottom: `1px solid ${alpha(theme.palette.divider, 0.1)}` }}>
          <Box sx={{ flex: 1 }}>
            {config && (
              <Chip
